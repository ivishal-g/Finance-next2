...
const app = new Hono()
  .get(
    ...
    async (c) => {
      ...
      const allCategories = Array.from(categoryMap.values()).sort((a, b) => b.value - a.value);
      ...
      for (const tx of allTransactions) {
        ...
        else current.expenses += Math.abs(tx.amount); // Use absolute value for expenses
      ...
      // Convert date string to Date object for fillMissingDays
      const finalActiveDaysWithDate = finalActiveDays.map((d) => ({
        ...d,
        date: new Date(d.date),
      }));

      const days = fillMissingDays(finalActiveDaysWithDate, startDate, endDate);
